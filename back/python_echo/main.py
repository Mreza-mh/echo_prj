...
def get_ml_predictor() -> MLRiskPredictor:
    # out: singleton MLRiskPredictor — مدل‌ها یک‌بار لود می‌شوند
...
def run_ml_analysis(patient_config: dict) -> dict | None:
    # in:  patient_config dict (age, sex, height, weight, ap_hi/lo, ...)
    # out: {hd_result, cv_result, combined_score, severity, risk_factors, ...} or None on error
    try:
